...
@testset "R^1 -> R^1" begin
    store = [0.0]
    f = x -> x^2*exp(x)
    jacf = (store, x) -> (store[1] = 2x*exp(x) + x^2*exp(x); return nothing)
    (_, _, _, pol) = checkDerivative_in_place(f, jacf, .64, store, nothing) #Float64 -> Float64
    @test coeffs(pol)[end] ≈ 2 atol = 3e-1

    f = x -> x[1]^2*exp(x[1])
    jacf = (store, x) -> (store[1] = 2x[1]*exp(x[1])+x[1]^2*exp(x[1]); return nothing)
...
@testset "R^3 -> R^1" begin
    ...
    ff = xx ->  xx[1]^2 * sin(xx[2]*xx[3])
    jacf = (store, xx) -> (store[1] = 2*xx[1]*sin(xx[2]*xx[3]); store[2] =  xx[1]^2*xx[3]*cos(xx[2]*xx[3]); store[3] = xx[1]^2*xx[2]*cos(xx[2]*xx[3]); return nothing)
...
@testset "R^1 -> R^3" begin
    store = [0.0, 0.0, 0.0]
    f = x -> [x;sin(x);log(x)]
    jacf = (store, x) -> (store[1] =1;store[2] = cos(x); store[3] = 1/x; return nothing)
    (_, _, _, pol) = checkDerivative_in_place(f, jacf, .4, store, nothing)
    @test coeffs(pol)[end] ≈ 2 atol = 3e-1 #input is scalar

    g = x -> [x[1];sin(x[1]);log(x[1])]
    jacg = (store, x) -> (store[1] = 1; store[2] = cos(x[1]); store[3] = 1/x[1]; return nothing) #input is 1x1 array
...
@testset "R^3 -> R^3" begin
    store = zeros(3, 3)
    f = x -> [x[1]x[2]x[3]; sin(x[1]) + x[2]exp(x[3]); exp(x[2] - x[3])/x[1]]
    jacf = (store, x) -> (store[1, 1] = x[2]x[3]; store[1, 2] = x[1]x[3]; store[1, 3] = x[1]x[2];
