...
"""
df is an in-place function (has no return value)
"""
function checkDerivative_in_place(f, df, x0, store, hessian = nothing, first = 3, last = 12, num = 10)
    if size(x0, 2) != 1
        x0 = reshape(x0, length(x0), 1) #reshape x0 into 2D column vector or 1D Array
    end
    f0 = f(x0)
    ...
    #println("size df0, 2: ", size(df0, 2))
    try
        @assert size(store, 1) == m && size(store, 2) == n
    catch e
        #@info "df0", store
        #@info "m", m
        #@info "n", n
        try
            store = reshape(store, m, n) # force reshape to m x n if non-scalar
        catch e
        end
    end
    if hessian!=nothing
    ...
    for i = 1:length(h)
        ...
        if false#debug
            ...
            if hessian!=nothing
                inc = h[i] .* dx
                expr1 = fi .- f0
                expr2 = store * inc .+ 0.5* inc' * d2f0 * inc
                #println("size expr1: ", size(expr1))
                #println("size expr2: ", size(expr2))
                @assert size(expr1, 1) == size(expr2, 1) && size(expr1, 2) == size(expr2, 2)
                A[i] = norm(expr1 .- expr2)
